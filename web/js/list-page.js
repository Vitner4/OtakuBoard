...
const totalCards = document.getElementById("total-cards") // Количество найденных карточек
const easterEgg = document.getElementById('easter-egg'); // Сообщение пасхалки

// Переменные
let selectedStar = 0; // Выбранная звезда рейтинга фильтра
const eeNum = Math.floor(Math.random() * (50 - 1 + 1)) + 1; // Пасхальное число 
...
async function loadCards() {
    ...
    try {
        ...
        if (!cards.length) {
            state.loading = false;
            totalCards.textContent =`Количество карточек: ${response.total}`;
